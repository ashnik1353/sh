pth = 'C:\code-1';
PTH(pth);
clear outputNB outputHMM outputHSMM  outputNHSMM outputCRF testLabelsSaves globPars
...
%%%%%%%%%%%%%%%%%%%ezf%%%%%%%%%%%%%%%%%%%%%%%
globPars.thau=15;
globPars.useCluster = 1;
globPars.numCluster = 4;
globPars.distType = 1; %0: levenstein 1:DTW 2:sameLevenstein
globPars.Itakura = 0.82;
globPars.subsWeight = 3;
globPars.relative = 0; %1:relative Levenstein
globPars.useBias = 0; %0:dont use 1:use
globPars.diffSize1 = 1; %0:same size1 1:different size1
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Iterate over experiment configs
    for k=1:length(conf{l}), 
        clear outputNB outputHMM outputHSMM outputCRF testLabelsSaves curExp outputNHSMM 
...
%% Perform inference for all models and days
        for i=1:globPars.stepDays:curSet.DSetInfo.numDays,
            ...
            outputNHSMM{index} = TrainTestDaysSplit('nhsmm', curExp);
...
%%      Save results and stats
       ...
       if (exist('outputNB'))
        ...
        if (exist('outputNHSMM'))
            res{l}{k}.statNHSMM = calcExtendedResultStats('nhsmm', dSet{l}, curSet, globPars, outputNHSMM, testLabelsSaves);
        end
        if (exist('outputNNHSMM'))
            res{l}{k}.statNNHSMM = calcExtendedResultStats('nnhsmm', dSet{l}, curSet, globPars, outputNNHSMM, testLabelsSaves);
        end
        if (exist('outputCRF'))
