function outTrain = nhsmmTrainDaysSplit(curExp)
...
numCluster = modelInfo.numCluster;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
learnedParams.obsModelHSMM = obsModel;
learnedParams.transModel = transModel;
learnedParams.durModel = durModel;
learnedParams.maxDur = maxDur;
%%%%%%%%%%%%%%%%%%%ezf%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

learnedParams.S = nsplitActivities(modelInfo, trainLabels, trainSegments, trainFeatMat);

if useCluster
    %learnedParams.medoids = nkmedoids(learnedParams.S, numCluster, thau, size1);
    [medoids,Distances] = nkmedoids(learnedParams.S, numCluster, modelInfo);
    learnedParams.medoids = medoids;
    
    %sigmas = zeros(length(actList),1);
    
    for i=1:length(actList),       
        sigmas{i}= ncalcSigma(Distances{i}, modelInfo);
    end
    learnedParams.sigmas = sigmas;    
end
learnedParams.trainFeatMat = trainFeatMat;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
